...
/**
 * Forced alignment using whisperx for perfect word-level timestamps.
 * Falls back to basic whisper if whisperx is not installed.
 *
 * @param {string} audioPath - Path to audio file (mp3, wav, etc.)
 * @param {string} outputPath - Path to write word-timings.json
 * @param {object} [options]
 * @param {string} [options.model="base"] - Whisper model for initial transcription
 * @returns {Promise<Array<{text: string, startMs: number, endMs: number}>>}
 */
export async function forcedAlignWordTimings(audioPath, outputPath, options = {}) {
  const model = options.model || "base";
  const absAudio = path.resolve(audioPath);
  const absOutput = path.resolve(outputPath);

  console.log(`  Running whisperx forced alignment (model: ${model})...`);

  // whisperx CLI: transcribe + align in one shot
  const cmd = `python3 -c "
...
audio = whisperx.load_audio('${absAudio}')
...
with open('${absOutput}', 'w') as f:
    json.dump(words, f, indent=2)

print(f'{len(words)} words aligned')
print(f'Last word ends at {words[-1][\"endMs\"]/1000:.1f}s' if words else 'No words')
"`;

  try {
    execSync(cmd, { stdio: "inherit", timeout: 120000 });
    const { readFileSync } = await import("fs");
    const wordTimings = JSON.parse(readFileSync(absOutput, "utf8"));
    console.log(`  ✅ ${wordTimings.length} words with forced alignment`);
    return wordTimings;
  } catch (err) {
    console.error(`  ⚠️  whisperx forced alignment failed: ${err.message}`);
    console.log("  Falling back to basic whisper...");
    const result = await transcribeAudio(audioPath, options);
    const { writeFileSync } = await import("fs");
    writeFileSync(absOutput, JSON.stringify(result.wordTimings, null, 2));
